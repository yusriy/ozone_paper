...
plot1 <- map +
         ...
         geom_text(data=s1,aes_string(x="longitude",y="latitude",label="station.id"),colour="black",size=4,fontface="bold",hjust=1.00,vjust=-1.00)+
         xlab("") + ylab("") +
...
plot2 <- map +
         ...
         geom_text(data=s2,aes_string(x="longitude",y="latitude",label="station.id"),colour="black",size=4,fontface="bold",hjust=1.25,vjust=1.25)+
         xlab("") + ylab("") +
...
plot3 <- map +
         ...
         geom_text(data=s3,aes_string(x="longitude",y="latitude",label="station.id"),colour="black",size=4,fontface="bold",hjust=1.25,vjust=0.5)+
         xlab("") + ylab("") +
...
png(filename = "D:/Site_Locations.png",height=4,width=12,
...
map1= map + geom_point (data = aqs,aes(x = longitude,y = latitude), 
      ...
      geom_text(data=aqs,aes_string(x="longitude",y="latitude",label="station.id"),colour="black",size=5,fontface="bold",hjust=1.1,vjust=0.25)+
...
oc <- readOGR(dsn="D:/Data/Boundaries/countries_shp", layer="countries")
...
p2<- ggplot()+geom_polygon(data=oc,aes(long,lat,group=group),fill="grey60")+
     ...
     plot.title = element_text(lineheight=0.5, face="bold",size =15, colour = "grey20"),
